...
type TreeContextProps = {
  ...
  direction: "rtl" | "ltr";
  onFileSelect?: (id: string, href?: string) => void;
};
...
const Tree = forwardRef<HTMLDivElement, TreeViewProps>(
  (
    ...
    const expandSpecificTargetedElements = useCallback(
      (nextElements?: TreeViewElement[], selectId?: string) => {
        if (!nextElements || !selectId) return;
        const findParent = (
    ...
    return (
      ...
      >
        <div className={cn("size-full", className)}>
          <div ref={ref} className="relative h-full overflow-y-auto px-2" dir={dir as Direction}>
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      ref={ref}
      className={cn(
        "absolute left-1.5 h-full w-px rounded-md bg-white/15 py-3 duration-300 ease-in-out hover:bg-white/30 rtl:right-1.5",
        className
      )}
      {...props}
    />
    ...
    return (
      ...
      >
        <AccordionPrimitive.Trigger
          className={cn(
            "flex items-center gap-1 rounded-md text-sm text-neutral-300 hover:text-white",
            className,
            {
...
>(
  (
    ...
    const { direction, selectedId, selectItem, onFileSelect } = useTree();
    const isSelected = isSelect ?? selectedId === value;
    return (
      <button
        ref={ref}
        ...
        disabled={!isSelectable}
        className={cn(
          "flex w-fit items-center gap-1 rounded-md pr-1 text-sm text-neutral-400 duration-200 ease-in-out hover:text-white rtl:pr-0 rtl:pl-1",
          {
